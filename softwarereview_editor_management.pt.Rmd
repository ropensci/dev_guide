...
- Novos editores são geralmente recrutados sempre que não há editores suficientes para lidar com os artigos enviados no momento, embora editores convidados também possam ser convidados para artigos específicos.

- A carga editorial atual pode ser consultada na [página _Editors_ do nosso painel de controle](https://dashboard.ropensci.org/editors.html).

### Procurando um novo editor {#searching-for-new-editors}

Alguns lugares onde o Líder de Revisão de Software pode começar a procurar novos editores incluem:

- A [página _Community Leaderboard_ o nosso painel editorial](https://dashboard.ropensci.org/community.html), para ajudar a identificar pessoas que contribuíram bastante, mas ainda não atuaram como editores.

- A lista de [*revisores e ex-editores*](#reviewers).

- Como alternativa, peça sugestões e indicações ao atual conselho editorial.

As decisões sobre novos editores devem, em geral, ser tomadas por toda a equipe, seguindo este procedimento geral:

- Crie um canal privado no Slack para discussão e adicione todos os editores.
  
  - É importante criar um novo canal privado para separar as discussões do canal geral dos editores, ao qual os futuros editores irão se juntar.
  
  - Envie uma menção no canal geral dos editores para garantir que todos sejam notificados sobre o novo canal privado.
  
  - Use esse canal para solicitar indicações gerais ou opiniões sobre possíveis candidatos.

- Você pode copiar e adaptar a seguinte mensagem para iniciar o canal:
  
  ````md
Todos vocês foram convidados para este canal a fim de discutir a inclusão de um novo membro em nosso conselho editorial.
...
Por favor, não façam capturas de tela e procurem evitar opiniões abertamente negativas.
   Em seguida, façam uma das seguintes opções:
   - Indiquem e descrevam brevemente qualquer candidato específico; ou
...
- Concentre-se em novos membros que irão sustentar a [diversidade](https://ropensci.org/blog/2025/02/05/no-science-without-deia/) e a representatividade global da equipe editorial.

- Aguarde até que a maioria dos editores se manifeste antes de convidar alguém. Dê cerca de uma semana para que respondam.

- Exclua o canal privado do Slack assim que o novo editor tiver aceitado o convite.

### Convidando um novo editor {#inviting-a-new-editor}

- Os membros do comitê editorial geralmente começam atuando como [editores convidados](#guesteditor) para uma submissão inicial de teste.
...
- Ao término do mandato como editor convidado, geralmente lhes é perguntado se gostariam de se tornar membros efetivos.

## Integrando um(a) novo(a) editor(a) ao time {#onboarding-a-new-editor}

- Adicionar ao [arquivo "team.json" do site](https://github.com/ropensci/roweb3/blob/main/data/team/team.json) com `"editor": true` e também `"stats": true` para editores de estatísticas.
  Isso adicionará automaticamente novos editores à equipe editorial mostrada na página do [site da rOpenSci](https://ropensci.org/software-review/).
...
- Convide-os para a organização rOpenSci no GitHub como membro da equipe  [`editors`](https://github.com/orgs/ropensci/teams/editors) e da equipe de editores de estatísticas [`stats-editors`](https://github.com/orgs/ropensci/teams/stats-editors).
  Isso dará a eles as permissões apropriadas e garantirá que apareçam no [painel de controle dos editores](https://dashboard.ropensci.org/editors).
...
## Colocando um editor em pausa {#pause-editor}

Às vezes, as pessoas editoras também precisam de uma pausa.
Para colocar um integrante da equipe editorial em pausa:
...
- Defina o status “férias” no Airtable, incluindo uma data aproximada de quando possam retornar.
...
- Atualizar o ["team.json" no site](https://github.com/ropensci/roweb3/blob/main/data/team/team.json) substituindo "roles" por "past\_roles" e adicionando um novo campo, `alumnus: "true"`.
