...
* __scientific software wrapper__: Packages that wrap command line clients used for scientific research. We strongly encourage wrapping open-source and open-licensed utilities. Wrappers must be non-trivial, in that there must be significant added value above just `system()` calls, whether in parsing inputs and outputs, data handling, etc. Improved installation, or extension of compatibility to more platforms, may constitute added value if installation is complex. Packages may run on limited platforms but only if they are performing a platform-specific task, or using other platform-limited utilities. Exceptions to these rules will be treated on a case-by-case basis. (Examples: [**babette**](https://github.com/ropensci/software-review/issues/208), [**nlrx**](https://github.com/ropensci/software-review/issues/262))
