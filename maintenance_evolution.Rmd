...
Now, renaming a package that has been widely adopted and/or released on CRAN is problematic as users might not hear about the new package.
If you decide to go forward with such renaming, make sure to acknowledge authors of the package under its original name by listing them in DESCRIPTION with a comment that states they were author of the original package.

Creating a newer package that is a rewrite with a better interface, like httr2 instead of httr ; or editions of a package like for thestthat, are different strategies.
