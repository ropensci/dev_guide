...
O EiC atua por 3 meses ou por um período combinado por todos os membros do conselho editorial. 
...
As informações sobre o status atual de todos os membros da equipe editorial são apresentadas em nosso [*Painel Editorial*](https://dashboard.ropensci.org).
Em detalhe, o EiC desempenha as seguintes funções
...
- Atribui submissões de pacotes a outros editores, inclusive a si mesmo, para que cuidem delas. Na maioria das vezes, isso é uma forma de rodízio entre os editores, a menos que o EiC considere que um editor seja particularmente adequado para um pacote ou que um editor se recuse a lidar com a submissão por estar muito ocupado ou por causa de conflito de interesse. 

- Para isso, use
  ```
  @ropensci-review-bot assign @username as editor
  ```
  Isso também adicionará a tag `1/editor-checks` à questão.

- Monitorar regularmente (por exemplo, semanalmente) o andamento de todas as revisões em aberto, acompanhando a página [*Painel*](https://dashboard.ropensci.org/reviews.html) e lembrando outros editores de avançar os pacotes conforme necessário.
...
O [*Painel Editorial da rOpenSci*](https://dashboard.ropensci.org) é atualizado diariamente, principalmente através da extração de informações sobre todas as questões relacionadas com a revisão de software no GitHub, juntamente com informações adicionais do Slack e da nossa base de dados Airtable.
O painel fornece uma visão geral atualizada de nossa equipe editorial, suas responsabilidades recentes e o estado atual de todas as questões relacionadas à revisão de software.
O EiC (ou qualquer editor interessado) pode obter uma visão geral do status da equipe editorial, disponibilidade e cargas de trabalho recentes na [página *editores*](https://dashboard.ropensci.org/editors.html).
...
- Analisar os envios em “0\/presubmission” e “1\/editorial-team-prep” para verificar se alguma ação precisa ser tomada (como consultar editores, tomar decisões, colocar questões em espera, solicitar atualizações ou encontrar e designar editores).

- Analisar os envios em “2\/seeking-reviewer(s)” para garantir que as coisas estejam progredindo rapidamente.
Se a busca pelo revisor estiver demorando mais do que o normal (cor vermelha), verifique se o envio está em espera, leia o tópico para entender o contexto e entre em contato com o editor em particular para pedir mais informações.
...
Em alguns casos, a documentação online é escassa. LEIAME simplificado e ausência de um site pkgdown dificultam a avaliação. 
Nesse caso, solicite mais detalhes; mesmo que o pacote seja considerado fora do escopo, a documentação do pacote terá melhorado, portanto, não há problema em adotar esses esforços.
...
Você se importaria de acrescentar mais detalhes e contexto ao LEIAME? 
Depois de lê-lo, alguém com pouco conhecimento do domínio deve ter sido informado sobre o objetivo, as metas e a funcionalidade do pacote.
...
- Verifique se o modelo foi preenchido corretamente. As omissões e falhas mais comuns devem ser detectadas e anotadas pelo bot, mas uma verificação manual sempre ajuda. Os editores podem editar os modelos diretamente para pequenas correções ou podem orientar os autores a preencher quaisquer campos obrigatórios do modelo que possam estar faltando.
...
- Use o [modelo de e-mail](#reviewrequesttemplate) se necessário, para convidar revisores. 
  - Ao convidar revisores, inclua algo como "se eu não tiver notícias suas em uma semana, presumirei que você não poderá fazer uma revisão", para dar um prazo claro no qual você irá procurar outra pessoa.
