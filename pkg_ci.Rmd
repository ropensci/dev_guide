...
For a package depending on Bioconductor packages, or a Bioconductor package, using `r: bioc-release` is the most appropriate but it will not flag issues with dependencies that won't install properly from the source code (which is the review scenario). Therefore, one should still also use `r: release` which can catch such problems. Including any setup instructions in the `script: filed` for that job then also allows testing the suitability of [given instructions](#pkgdependencies). 
