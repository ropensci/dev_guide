...
La [suite de paquetes](https://ropensci.org/packages/) de rOpenSci recibe aportes tanto de miembros de la organización y como por miembros de la comunidad, lo que significa que se nutre de la gran diversidad de habilidades y experiencias.
...
Puedes reconocer los paquetes de rOpenSci que han sido revisados por pares mediante una etiqueta verde que lee "peer-reviewed" en su README (p.e. el paquete [restez](https://github.com/ropensci/restez#locally-query-genbank-)), y por el icono azul en su descripción en [la página de paquetes de rOpenSci](https://ropensci.org/packages/), ambos contienen links a la revisión.
...
Además, utilizamos funciones de GitHub como las plantillas de issues (como plantilla de envío), y el etiquetado que utilizamos para seguir el progreso de los envíos (desde las revisiones del editor hasta la aprobación).

## ¿Por qué enviar tu paquete a rOpenSci? {#whysubmit}

-   En primer lugar, y sobre todo, esperamos que envíes tu paquete para su revisión **porque valoras la devolución**. Nuestro objetivo es proporcionar una devolución útil a las/os autoras/es de paquetes y que nuestro proceso de revisión sea abierto, no conflictivo y centrado en la mejora de la calidad del software.
...
-   Si escribes un *gitbook relacionado a tu paquete*, rOpenSci lo promoverá: el código fuentel mismo puede ser transferido a [la organización de GitHub `ropensci-books`](https://github.com/ropensci-books) para ser listado [en books.ropensci.org](https://books.ropensci.org/).
...
-   La revisión es una conversación bidireccional. Al revisar los paquetes, tendrás la oportunidad de **seguir aprendiendo buenas prácticas de desarrollo de las/os autoras/es y otras/os revisoras/es**.
-   La naturaleza abierta de nuestro proceso de revisión te permite **establecer redes y conocer colegas y colaboradoras/es**. Nuestra comunidad es amigable y está llena de miembros con conocimiento en el desarrollo de R y en muchas otras áreas de la ciencia y la informática científica con ganas de ayudar.
...
Las/os autoras/es, revisoras/es y editoras/es conocen la identidad del resto y la comunidad en general puede ver o incluso participar en la conversación a medida que se produce.
...
También es posible que las/os revisoras/es sean menos críticos en la revisión abierta.
...
En definitiva, creemos que tener una comunicación directa y pública entre autoras/es y revisoras/es mejora la calidad y la imparcialidad de las revisiones.

Además, tanto autoras/es como revisoras/es tienen la posibilidad de contactar a las/os editoras/es por privado si tienen alguna duda o pregunta.
...
-   Tu paquete tendrá un [sitio web de docs en `docs.ropensci.org`](#rodocsci) que podrás enlazar en el DESCRIPTION.
-   El repositorio de tu paquete será transferido a la organización rOpenSci.
-   Si las/os revisoras/es [aceptan aparecer en el DESCRIPTION](#authorship) sus metadatos mencionarán la revisión.

## Editoras/es y revisoras/es

### Editoras/es asociadas/os {#associateditors}
