...
* If there is potential overlap or confusion with other packages providing similar functionality or having a similar name, add a note in the README, main vignette and potentially the Description field of DESCRIPTION. Example in [rtweet README](https://docs.ropensci.org/rtweet/), [rebird README](https://docs.ropensci.org/rebird/#auk-vs-rebird).
...
Many packages include code from other software. Whether entire files or single functions are included from other packages, rOpenSci packages must follow [the CRAN *Repository Policy*](https://cran.r-project.org/web/packages/policies.html):
...
* If your package interacts with web resources (web APIs and other sources of data on the web) you might find the [HTTP testing in R book by Scott Chamberlain and Maëlle Salmon](https://books.ropensci.org/http-testing/) relevant. Packages helping with HTTP testing (corresponding HTTP clients):
    ...
    * [httptest](https://enpiar.com/httptest/) ([httr](https://httr.r-lib.org/));
...
* Consider the trade-offs involved in relying on a package as a dependency. On one hand,
  ...
  dependencies:
    ...
    * Small, simple functions from a dependency package may be better copied into
      ...
      in an otherwise large or heavy dependency.  (See [*Authroship* section
