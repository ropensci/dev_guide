...
- For HTTP requests we recommend using [httr2](https://httr2.r-lib.org), [httr](https://httr.r-lib.org), or [curl](https://jeroen.r-universe.dev/curl#) over [RCurl](https://cran.rstudio.com/web/packages/RCurl/). (And rOpenSci's [crul](http://docs.ropensci.org/crul/) package is no longer maintained.) If you like low level clients for HTTP, curl is best, whereas httr2 and httr are better for higher level access.
