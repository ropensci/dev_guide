...
* If your (not Bioconductor) package depends on Bioconductor packages, make sure the installation instructions in the README and vignette are clear enough.
    
    * Is the automatic installation of Bioconductor packages by `install.packages()` enough? In that case, mention the user needs to run `setRepositories()` if they haven't set the necessary Bioconductor repositories yet. 
    
    * Should the user use [`BiocManager`](https://www.bioconductor.org/install/index.html#why-biocmanagerinstall)? Document this.
