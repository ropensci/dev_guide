# Guía para autores/as {#authors-guide}

```{block, type='summaryblock'}
Esta guía condensa el proceso de revisión por pares desde el punto de vista del/a autor/a de paquetes. 
```

## Planificar un envío (o una consulta previa al envío)

-   ¿Esperas mantener tu paquete durante al menos 2 años, o poder identificar a un nuevo encargado/a?
-   Consulta nuestras [políticas](#policies) para evaluar si tu paquete cumple nuestros criterios para encajar en nuestro conjunto de paquetes y no se superpone con otros paquetes.
...
-   Por favor, considera cuál es el mejor momento en el desarrollo de tu paquete para presentarlo. Tu paquete debe estar lo suficientemente maduro como para quienes lo revisen puedan evaluar todos los aspectos esenciales, pero ten en cuenta que la revisión puede resultar en cambios importantes.
-   Te recomendamos encarecidamente que envíes tu paquete para su revisión *antes* de publicar en CRAN o enviar un artículo de software describiéndolo a una revista. Es posible que tu paquete sufra mejoras importantes y actualizaciones fruto de la revisión, incluyendo cambios en el nombre de las funciones o incompatibilidades con versiones anteriores.
-   No envíes tu paquete para su revisión mientras éste o un manuscrito asociado estén también en proceso de revisión en otro lugar, ya que esto puede dar lugar a solicitudes de cambios contradictorias.
-   Ten en cuenta también el tiempo y el esfuerzo necesarios para responder a las revisiones: piensa en tu disponibilidad o en la de tus colaboradores/as en las semanas y meses siguientes al envío. Ten en cuenta que las revisiones se hacen por voluntarios/as, y te pedimos que respetes su tiempo y esfuerzo respondiendo puntual y respetuosamente.
-   Si utilizas [etiquetas de repostatus.org](https://www.repostatus.org/) (que recomendamos), envía tu paquete cuando esté listo para obtener la etiqueta *Active* en vez de *WIP*. Si utilizas [etiquetas del ciclo de vida](https://www.tidyverse.org/lifecycle/), el envío debe producirse cuando el paquete esté al menos en el estado *Maturing*.
-   Para cualquier envío o consulta previa al envío, el README de tu paquete debería proporcionar suficiente información sobre el mismo (objetivos, uso, paquetes similares) para que los/as revisores/as puedan evaluar su alcance sin tener que instalar el paquete. Mejor aún, crea un sitio web de pkgdown para que puedan evaluar la funcionalidad detalladamente en línea.
...
-   El archivo README debe esforzarse por explicar la funcionalidad y los objetivos de tu paquete asumiendo poco o ningún conocimiento de dominio. Además, debe aclarar todos los temas técnicos, incluidas las referencias a otro software.
...
-   La comunicación entre autores/as, revisores/as y editores/as se dará principalmente en GitHub, para que el issue de revisión sirva de registro completo de la misma. Puedes contactarte con el/la editor/a por correo electrónico o Slack si es necesario realizar una consulta privada (por ejemplo, preguntar cómo responder a una pregunta de un/a revisor/a). No te pongas en contacto con los/as revisores/as fuera del issue sin antes preguntarles, dentro del mismo, si están de acuerdo con ello.
...
-   Los paquetes se checkean automáticamente por [nuestro sistema `pkgcheck` sistema](https://docs.ropensci.org/pkgcheck) al ser enviados, el cual confirmará si está listo para ser revisado o no.
-   Los paquetes enviados pueden estar en la branch main/default, o en cualquier otra rama no predeterminada. En este último caso, es recomendable, aunque no obligatorio, enviar el paquete a través de una branch dedicada llamada `ropensci-software-review`.

## El proceso de revisión

-   Un/a [editor/a](#editors) revisará tu envío en un plazo de 5 días laborables y te responderá con los siguientes pasos a seguir. Puede asignar el paquete a los/as revisores/as, solicitar que el paquete se actualice para cumplir los criterios mínimos antes de la revisión, o rechazar el paquete porque el mismo no encaja en rOpenSci o porque se solapa con uno ya existente.
-   Si tu paquete cumple los criterios mínimos, se le asignará de 1 a 3 revisores/as, a quienes se les pedirá proporcionar revisiones en forma de comentarios sobre tu issue en un plazo máximo de 3 semanas.
-   Te pedimos que respondas a estos comentarios en un plazo máximo de 2 semanas desde la última revisión presentada, pero puedes actualizar tu paquete o responder en cualquier momento. Tu respuesta debe incluir un enlace a la actualización del [NEWS.md](#news) de tu paquete. Aquí tienes [un ejemplo de respuesta](https://github.com/ropensci/software-review/issues/160#issuecomment-355043656). Fomentamos las conversaciones continuas entre autores/as y revisores/as. Consulta la [guía de revisión](#reviewerguide) para más detalles.
-   Si algún cambio en el paquete puede modificar los resultados de [`pkgcheck`](https://docs.ropensci.org/pkgcheck), se puede solicitar un nuevo check con el comando `@ropensci-review-bot check package`.
