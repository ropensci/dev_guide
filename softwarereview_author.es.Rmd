...
- Si utilizas [etiquetas de repostatus.org](https://www.repostatus.org/) (que recomendamos), envía tu paquete cuando esté listo para obtener la etiqueta *Active* (activo) en vez de *WIP* (_Work in Progress_, Trabajo en proceso). Si utilizas [etiquetas de ciclo de vida](https://lifecycle.r-lib.org), el envío debe producirse cuando el paquete esté al menos en el estado *Stable* (estable).
...
- Tal como se indica en [nuestras políticas generales](#policies-ai), exigimos a todos los autores que envíen trabajos que proporcionen detalles sobre cualquier uso de herramientas de IA generativa en la documentación del paquete, generalmente en los archivos README o en las guías de contribución.
...
- Sugerimos utilizar las herramientas de IA de manera que se registren las decisiones que guían los procesos de desarrollo y los resultados dentro del propio software.
  - Entre los ejemplos de este tipo de herramientas se encuentran desde [el «spec-kit» de GitHub para el desarrollo basado en especificaciones](https://github.com/github/spec-kit) hasta [nuestra propia herramienta experimental para documentar las decisiones de diseño de software](https://github.com/ropensci-review-tools/designlens).
  - Todas ellas comparten el patrón común de generar artefactos adicionales que documentan las decisiones y los procesos de desarrollo junto con el software en sí.
...
- Si tu paquete requiere dependencias inusuales del sistema (ver [*Guía de Empaquetado*](#pkgdependencies) para que nuestra Acción de GitHub pase, por favor envía un _pull request_ añadiéndolas a [nuestro _Dockerfile_ base](https://github.com/ropensci-review-tools/pkgcheck/blob/main/Dockerfile).
  Consulta [esta viñeta `pkgcheck`](https://docs.ropensci.org/pkgcheck/articles/environment.html) para obtener más información sobre nuestro entorno de comprobación y cómo modificarlo para que tu paquete pase las verificaciones.
