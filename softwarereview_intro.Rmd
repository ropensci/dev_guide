...
_If you use our standards/checklists etc. when reviewing software elsewhere, do tell the recipients (e.g. journal editors, students, internal code review) that they came from rOpenSci, and tell us in [our public forum](https://ropensci.org/usecases/), or [privately by email](https://ropensci.org/contact/)._ 
```
...
Information on the current team, and the current status of software peer review, can be seen on our [interactive dashboard](https://dashboard.ropensci.org).
...
if (nzchar(Sys.getenv("AIRTABLE_API_KEY"))) {
  source(
    file.path("scripts", "airtable-get-eic.R"),
    local = knitr::knit_global()
  )
} else {
...
### Reviewers and former editors {#reviewers}
