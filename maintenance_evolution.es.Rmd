...
Cuando usen `z`, como no es un parámetro en la definición de la función, y es probable que sea capturado por los `...` y seea ignore silenciosamente -- no es lo que quieres.
En su lugar, deja el argumento, emitiendo un error si se utiliza.
...
También es útil tener un mensaje, pero entonces sólo querrás emitirlo cuando se la función que va a desaparecer, por ejemplo
...
En la propia función, utiliza `.Deprecated()` para indicar la nueva función que substituye la anterior. 
...
Esto crea una página de manual a la que los usuarios pueden acceder como `` ?`holamundo-deprecated` `` y verán en el índice de documentación.
...
Haz una página de manual para las funciones caducas como

``` r
#' Funciones caducas en helloworld
#' 
#' Estas funciones han desaparecido, ya no están disponibles.
#'
#' \itemize { 
#'   \item \code{\link{foo}}:  Esta función ha desaparecido.
#' }
#' 
...
Esto crea una página de manual que se accede como `` ?`helloworld-defunct` `` y que aparece en el índice de documentación.
...
Los paquetes archivados son [archivados](https://docs.github.com/en/repositories/archiving-a-github-repository/archiving-repositories) y transladados a una organización dedicada de GitHub, [ropensci-archive](https://github.com/ropensci-archive).
