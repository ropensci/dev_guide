...
While we currenly have no formal requirements for how tool usage should be documented, examples include tools which produce systematic records of design decisions and histories, such as [GitHub's "spec-kit" for specification-driven development](https://github.com/github/spec-kit) or [our own experimental 'designlens' tool to document software design decisions](https://github.com/ropensci-review-tools/designlens).
Both of these create an additional `specs/` directory intended to be committed within a repository.
That directory records individual development phases, generally under sequentially-numbered sub-directories, with each containing several documents like "plan.md", "tasks.md", and "report.md" or some equivalent summary of that step.
A `specs/` folder can then contain a comprehensive history of software design, and provide an informative basis for review.
