...
Note that when installing the package to review it, you should use the `dependencies = TRUE` argument of `remotes::install()` to make sure you have all dependencies available. If the package has changed substantially since the last [automated `pkgcheck` checks](https://docs.ropensci.org/pkgcheck), you may request a re-check with the command `@ropensci-review-bot check package`.
