...
Guidance for when you develop a package interacting with a web resources asking for credentials (API keys, tokens, etc.). Also refer to [the `httr` vignette about sharing secrets](https://httr.r-lib.org/articlaes/secrets.html).
...
You'll need to protect your secrets as you protect your users' secrets.

#### Secrets and recorded requests in tests

If you use `vcr` or `httptest` in tests for caching API responses, you need to make sure the fixtures you save do not contain secrets. Refer to [`vcr` guidance](https://books.ropensci.org/http-testing/security.html) and [`httptest` guidance "Redacting and Modifying Recorded Requests"](https://enpiar.com/r/httptest/articles/redacting.html).

#### Share secrets with CI services

Now, you might need to share secrets with CI services.

* You could store API keys as environment variables on Travis and on GitHub.

* For bigger secrets like tokens, refer [to the googlesheets guidance](https://cran.r-project.org/web/packages/googlesheets/vignettes/managing-auth-tokens.html#the-making-of-googlesheets.tar.gz-for-cran) that involves in particular the encryption of tokens for sharing them with Travis.
...
Note that you don't have to skip tests that use recorded requests (with e.g. `vcr` cassettes).
