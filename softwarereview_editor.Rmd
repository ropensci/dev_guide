...
#### Deciding on scope and overlap {#outofscoperesponse}
...
- Issue the following command to email all editors a notification that we're seeking volunteers for the submission:
  ...
  EiC's may automatically assign editors to issues, but may first like to ask directly via Slack to ensure editors are willing to be assigned.
...
#### rOpenSci staging universe

Once the package gets the editor-checks label, you should see its build on the [ropensci-staging R-universe](https://ropensci-staging.r-universe.dev/builds).
This might be helpful as it allows you to gauge:
- (in the main table) whether it can be built on various OSs;
- (in the main table) whether a pkgdown website can be built for it via the book icon (websites are not actually deployed until packages are approved);
- (on the individual package page) its dependencies and other info. You can take the website tour (button in the navbar) if you are not familiar with [R-universe](https://docs.r-universe.dev/).
...
- After automatic checks are posted, use the [editor template](#editortemplate) to guide initial checks (if not already covered by the EiC) and record your response to the submission.
  Prefix each of your comments with an [identifier consisting of your initials and a number](#prefixing) e.g. `ml01`, `ml02`, etc.
...
- If initial checks show major gaps, request changes before assigning reviewers.
  ...
  You'll get a reminder in the issue every 90 days to check in with the package author(s). The holding label can be manually removed in the issue's sidebar. 
