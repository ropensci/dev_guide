...
donde issue\_id es el número de la cuestión en el repositorio de revisión de software. Por ejemplo, la insignia de [`rtimicropem`](https://github.com/ropensci/rtimicropem) revisión utiliza el número 126, ya que es el número de [número de edición de revisión](https://github.com/ropensci/software-review/issues/126). La insignia indicará primero "en revisión" y luego "revisado" una vez que tu paquete haya sido incorporado (cuestión etiquetada como "aprobada" y cerrada), y enlazará con la cuestión de revisión.
...
- Si tu README tiene muchas insignias, considera la posibilidad de ordenarlas en una tabla html para facilitar a los recién llegados la obtención de información de un vistazo. Consulta los ejemplos en [`drake` repo](https://github.com/ropensci/drake) y en [`qualtRics` repo](https://github.com/ropensci/qualtRics/). Las secciones posibles son
  ...
  - Desarrollo (estados de CI cf [capítulo CI](#ci)canal de Slack para la discusión, repostatus)
...
- Recomendamos no crear `README.md` directamente, sino a partir de un archivo `README.Rmd` archivo (un archivo R Markdown) si tienes algún código de demostración. La ventaja del archivo `.Rmd` es que puedes combinar el texto con el código que se puede actualizar fácilmente cada vez que se actualice tu paquete.

- Considera la posibilidad de utilizar `usethis::use_readme_rmd()` para obtener una plantilla para un archivo `README.Rmd` y para configurar automáticamente un gancho de precompromiso que garantice que `README.md` sea siempre más reciente que `README.Rmd`.
...
- Consulta el [`gistr` README](https://github.com/ropensci/gistr#gistr) para ver un buen ejemplo de README a seguir para un paquete pequeño, y [`bowerbird` README](https://github.com/ropensci/bowerbird) para un buen ejemplo de README para un paquete más grande.
...
- El paquete debe contener la documentación de nivel superior para `?foobar`(o ``?`foobar-package` `` si hay un conflicto de nombres). Opcionalmente, puedes utilizar tanto `?foobar` y ``?`foobar-package` `` para el archivo de manual de nivel de paquete, utilizando la etiqueta `@aliases` etiqueta roxygen. `usethis::use_package_doc()` añade la plantilla para la documentación de nivel superior.

- El paquete debe contener al menos un archivo **HTML** que proporcione una cobertura sustancial de las funciones del paquete, ilustrando casos de uso realistas y cómo se pretende que interactúen las funciones. Si el paquete es pequeño, la viñeta y el README pueden tener un contenido muy similar.
...
- Pedimos a todos los envíos que utilicen `roxygen2` para la documentación.  `roxygen2` es [un paquete de R](https://cran.r-project.org/web/packages/roxygen2/index.html) que compila automáticamente `.Rd` archivos a su `man` carpeta de su paquete a partir de simples etiquetas escritas sobre cada función.
...
- Una ventaja clave de utilizar `roxygen2` es que tu `NAMESPACE` siempre se generará automáticamente y estará actualizada.

- Todas las funciones deben documentar el tipo de objeto devuelto bajo el `@return` encabezamiento.

- El valor por defecto de cada parámetro debe estar claramente documentado. Por ejemplo, en lugar de escribir `A logical value determining if ...`deberías escribir ``A logical value (default `TRUE`) determining if ...``. También es una buena práctica indicar los valores por defecto directamente en la definición de tu función:
...
- La documentación debe apoyar la navegación del usuario incluyendo enlaces cruzados útiles entre funciones relacionadas y documentando funciones relacionadas juntas en grupos o en páginas de ayuda comunes.  Las páginas `@seealso` y `@family` (esta última crea automáticamente enlaces "Ver también" y [pueden ayudar a agrupar](https://pkgdown.r-lib.org/reference/build_reference.html) funciones juntas en sitios pkgdown), se recomiendan para este propósito. Consulta [la sección "manual" del libro The R Packages](https://r-pkgs.org/man.html) y [la sección "agrupación de funciones" del presente capítulo](#function-grouping) para más detalles.

- Podrías reutilizar piezas de documentación (por ejemplo, detalles sobre la autentificación, paquetes relacionados) en las viñetas/README/páginas de manual. Un posible enfoque para ello es el uso de [fragmentos de R Markdown, confiando en el uso de knitr de los documentos hijos para que puedas almacenar las partes reutilizadas en una carpeta en man/, y llamarlas tanto desde el README como desde la viñeta](https://www.garrickadenbuie.com/blog/dry-vignette-and-readme/) como desde [como en las páginas man utilizando la etiqueta `@includeRmd` disponible en `roxygen 0.7.0`](https://roxygen2.r-lib.org/articles/rd.html#including-external--rmd-md-files).

- Añade `#' @noRd` a las funciones internas.

- Si prefieres no abarrotar el código con una extensa documentación, puedes colocar más ejemplos fuera del script de R y utilizar la etiqueta `@example` y colocar más documentación en archivos en una carpeta `man-roxygen` carpeta en la raíz de tu paquete, y éstos se combinarán en el archivo del manual mediante el uso de `@template <file name>`por ejemplo.
  
  - Coloca cualquier documentación de un objeto en un archivo `.R` archivo en la carpeta `man-roxygen` (en la raíz de tu paquete). Por ejemplo [este archivo](https://github.com/ropensci/rgbif/blob/master/man-roxygen/occ_search_egs.R). Enlaza con ese archivo de plantilla desde tu función ([por ejemplo](https://github.com/ropensci/rgbif/blob/master/R/occ_search.r)) con la palabra clave `@template` palabra clave ([e.g.](https://github.com/ropensci/rgbif/blob/master/R/occ_search.r#L7)). El contenido de la plantilla se insertará cuando se construya la documentación en el archivo resultante `.Rd` que los usuarios verán cuando pidan la documentación de la función.
  ...
  - En la mayoría de los casos puedes ignorar las plantillas y `man-roxygen`pero hay dos casos en los que aprovecharlos será de gran ayuda:
    1. Cuando tengas mucha documentación para una función/clase/objeto, separar ciertos trozos de esa documentación puede mantener tu `.R` archivo fuente ordenado. Esto es especialmente útil cuando tienes mucho código en ese `.R` archivo. Por otro lado, significa que la documentación no es legible "en el código fuente", ya que está en otro archivo.
    2. Cuando tienes las mismas partes de la documentación utilizadas en muchas `.R` funciones, es útil utilizar una plantilla. Esto reduce el texto duplicado y ayuda a evitar que se actualice por error la documentación de una función pero no la de otra. Utilizar un archivo de plantilla para la documentación de un parámetro es una alternativa a definir el parámetro para una función y utilizar `@inheritParams` para otras funciones que utilicen el mismo parámetro.

- A partir de la `roxygen2` versión 7.0.0, `R6` las clases son oficialmente compatibles. Consulta la [roxygen2 docs](https://roxygen2.r-lib.org/articles/rd.html#r6) para saber cómo documentar las `R6` las clases.
