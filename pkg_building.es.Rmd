...
- Evita los conflictos de nombres de funciones con paquetes base u otros populares (por ejemplo `ggplot2`, `dplyr`, `magrittr`, `data.table`)
...
- Puedes elegir utilizar `=` en lugar de `<-` siempre que seas coherente con una elección dentro de tu paquete. Recomendamos evitar el uso de `->` para la asignación dentro de un paquete. Si utilizas `<-` en todo tu paquete, y también utilizas `R6` en ese paquete, te verás obligado a utilizar `=` para la asignación dentro de tu `R6Class` construcción - esto no se considera una incoherencia porque no puedes usar `<-` en este caso.
...
- Todos los paquetes deben tener un archivo README, llamado `README.md`en la raíz del repositorio. El README debe incluir, de arriba a abajo
  ...
  - Cualquier configuración adicional que se requiera (fichas de autenticación, etc.).
...
- Puedes ejecutar los ejemplos con `devtools::run_examples()`. Ten en cuenta que cuando ejecutes R CMD CHECK o su equivalente (p. ej, `devtools::check()`) tus ejemplos que no estén envueltos en `\dontrun{}` o `\donttest{}` se ejecutan. Consulta la [tabla resumen](https://roxygen2.r-lib.org/articles/rd.html#functions) en la documentación de roxygen2.
