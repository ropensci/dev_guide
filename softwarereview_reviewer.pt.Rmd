...
Convidamos qualquer pessoa que esteja lendo isto a preencher nosso [formulário de voluntariado](https://ropensci.org/software-reviewer) a qualquer momento.
Se você já aceitou o convite de realizar uma revisão, assim que for designado(a) para uma *issue* de revisão, nosso bot também fornecerá o link e solicitará automaticamente que você preencha o formulário.
...
Para revisar um pacote, comece copiando nosso [modelo de revisão](#reviewtemplate) ou o [modelo de revisão para software estatístico](https://stats-devguide.ropensci.org/pkgreview.html#pkgrev-template).
Esses modelos devem ser usados como listas de verificação de alto nível para guiar a estrutura da revisão inicial.
...
#### Formatação {#prefixing}

Você pode estar escrevendo muitos comentários individuais como parte da sua revisão.
Para facilitar a comunicação em torno deles, por favor, coloque um prefixo nas suas respostas da seguinte forma: suas iniciais — a menos que você as compartilhe com outra pessoa na discussão — e um número:
...
Assim como esperamos dos(as) autores(as), exigimos que os(as) revisores(as) divulguem qualquer uso de ferramentas de IA generativa com o máximo de detalhes possível.
Exemplos de uso aceitável incluem a geração de uma visão geral das funcionalidades de um pacote ou a análise do histórico do Git para fornecer uma visão geral das decisões de desenvolvimento.
