...
#### Decidir el alcance y el solapamiento {#outofscoperesponse}
...
- Ejecuta el siguiente comando para enviar por correo electrónico a todas las personas editoras una notificación indicando que estamos buscando quien se haga cargo de la edición:
  ...
  Quienes integran el equipo editorial pueden manifestar su interés haciendo clic en un enlace que aparece en su correo electrónico.
  Cada clic enviará automáticamente un correo electrónico de notificación al editor o editora en jefe (EiC) informándole quién ha hecho clic.
  Quienes desempeñan el rol de EiC pueden asignar automáticamente a integrantes del equipo editorial a los _issues_, pero quizá prefieran consultarles primero por Slack para confirmar que aceptan la asignación.

- Si nadie responde, podría ser necesario realizar una convocatoria más activa.
...
#### Universo de pruebas de rOpenSci

Una vez que el paquete reciba la etiqueta «editor-checks», deberías ver su compilación en el 
 [R-universe «ropensci-staging»](https://ropensci-staging.r-universe.dev/builds)   .
   Esto puede resultarte útil, ya que te permite evaluar:
- (en la tabla principal) si se puede compilar en distintos sistemas operativos;
- (en la tabla principal) si se puede crear una página web con pkgdown a través del icono del libro (las páginas web no se publican hasta que se aprueban los paquetes);
- (en la página del paquete en cuestión) sus dependencias y otra información. Puedes hacer el recorrido por la web (el botón de la barra de navegación) si no estás familiarizado con 
...
- Después de contabilizar las comprobaciones automáticas, utiliza la [plantilla editorial](#editortemplate) para guiar las comprobaciones iniciales (si no están ya cubiertas por quien ocupa el rol de LE) y registrar tu respuesta al envío.
  Empieza todos tus comentarios con un identificador formado por tus iniciales y un número, por ejemplo: ml01, ml02, etc.
...
- Si los chequeos iniciales muestran falencias importantes, solicita cambios antes de asignar personas para la revisión.
  ...
  Recibirás un recordatorio cada 90 días (en el *issue*) para que te pongas en contacto con las personas a cargo del paquete. La etiqueta de «en espera» se puede eliminar manualmente en la barra lateral del _issue_.
...
![Captura de pantalla de la interfaz de Airtable con el enlace 'Search Reviewers'](images/airtable.png)
