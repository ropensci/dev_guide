...
Si estás en el rol de edición de forma invitada, ¡gracias por ayudar! 
Ponte en contacto on la persona que te invitó a encargarte del proceso de revisión de un paquete.
...
El rol de LE es responsable de [la dirección general](#eic-general) del proceso de revisión, y de la [fases iniciales](#eic-submission) de todos los envíos.

### Funciones generales del rol de LE {#eic-general}

Se encarga de la gestión general de todas las [cuestiones de revisión del software](https://github.com/ropensci/software-review/issues?q=sort%3Aupdated-desc%20is%3Aissue%20is%3Aopen) con la ayuda de nuestro [panel editorial](https://dashboard.ropensci.org) como se describe a continuación [subsección del panelcontrol](#eic-dashboard).
Las responsabilidades del rol de LE incluyen las siguientes tareas generales:

- Al comienzo de una rotación, debe revisar el estado de las revisiones abiertas actuales en el [dashboard editorial](https://dashboard.ropensci.org/reviews.html) y enviar recordatorios a editores o autores de paquetess según sea necesario.

- Vigila todos los *issues* publicados en el repositorio de revisión del software, para lo cual debe suscribirse a las notificaciones del repositorio en GitHub, y también vigila el repositorio de revisión del software. `#editors-only` en Slack.

- Supervisa regularmente (por ejemplo, semanalmente) el ritmo de todas las revisiones abiertas, echando un vistazo al [dashboard](https://dashboard.ropensci.org/reviews.html) y recordando a otros editores que muevan los paquetes según sea necesario.

### Tareas del rol de LE para cada envío inicial {#eic-submission}

El rol de LE es responsable de la tramitación inicial de todos los nuevos envíos.
...
2. Proceder a un envío completo y asignar alguién del equipo editorial al envío.

#### Decidir el alcance y el solapamiento

Tiene derecho a tomar decisiones de alcance y solapamiento con la mayor independencia posible (pero puede solicitar ayuda/asesoramiento)., pero se le recomienda que solicite opiniones sobre el `#editors-only` en Slack.
Las decisiones sobre el alcance del software estadístico suelen ser más fáciles que las de las propuestas generales (no estadísticas), ya que [se describe a continuación](#eic-stats-submissions).
...
- Referirse a las categorías descritas en el [*Objetivos y ámbito de aplicación*](#aims-and-scope) para tomar decisiones sobre el alcance y el solapamiento de las consultas previas a la presentación, las remisiones de JOSS u otros socios de publicación, y las presentaciones.
  ...
  - El software estadístico debe considerarse dentro del ámbito de aplicación siempre que pueda cumplir al menos la mitad de todos los [las normas aplicables (generales y al menos una categoría)](https://stats-devguide.ropensci.org/standards.html).

- Si se considera que una consulta previa al envío o un envío está fuera de alcance de publicación, si procede.
  Cuando proceda, utiliza el texto de [*Objetivos y alcance*](#aims-and-scope) sobre la evolución del ámbito de aplicación a lo largo del tiempo.
  ...
  - Tras explicar una decisión fuera del ámbito de aplicación, escribe un comentario sobre la cuestión `@ropensci-review-bot out-of-scope`.

- Si una consulta previa a la presentación se considera dentro del alcance, el EiC *puede* realizar comprobaciones preliminares.
  En [*Plantilla de editores*](#editortemplate) puede utilizarse para ello.
...
La persona con el rol de LE debe asegurarse mínimamente de que la documentación es suficiente para juzgar el alcance, incluido un sitio web adjunto.
...
<opcional>
Si un paquete tiene funcionalidad que se solapa con otros paquetes, requerimos que demuestre en la documentación [por qué es único en comparacion con otros paquetes similares](https://devguide.ropensci.org/policies.html#overlap). Podrías añadir una comparación más detallada con los paquetes que mencionas en el *README* para que podamos evaluarlo?
</opcional>


#### Tareas iniciales de rol de LE para envíos completos

- Una vez que esté convencido de que un paquete puede pasar a un envío completo, invita un envío completo y, a continuación, cierra la consulta previa a la presentación.

- Etiqueta inicialmente cada nuevo envío completo con ` 0/editorial-team-prep`

- Encuentra alguién del equipo editorial para que se encargue del envío (incluido tú mismo, potencialmente).
  Quién es disponible actualmente se indica en la página [*Panel Editorial*](https://dashboard.ropensci.org), y las cargas de trabajo editoriales deben distribuirse de la forma más uniforme posible, remitiéndose al [*Cuadro de envios* gráficos de carga editorial reciente](https://dashboard.ropensci.org/editors.html#past-ed-load).
  También se puede invitar a un editor invitado para que se encargue de cualquier envío, tal y como se describe en el [subsección siguiente](#guesteditor).
...
El software estadístico debe considerarse dentro del ámbito de aplicación siempre que cumpla > 50% de todos los [las normas aplicables](https://stats-devguide.ropensci.org/standards.html).
Si una consulta previa a la presentación indica que ya se han cumplido las normas, la persona con el rol de LE debe:

- Usa `@ropensci-review-bot check srr` para confirmar que se han cumplido las normas.
- Considera si el paquete está mejor situado en la categoría estadística designada, o si podrían ser adecuadas categorías alternativas o adicionales.

Si aún no se han cumplido las normas, se debe pedir a quién envió el paquete que juzguen si creen que su paquete podrá cumplir al menos la mitad de todas las normas generales y específicas de la categoría.
Esto puede requerir un debate sobre la categoría o categorías adecuadas.
Si las autoras no han cumplido las normas pero estan de acuerdo en hacerlo, por lo general debe aplicarse una etiqueta de "holding" hasta que `@ropensci-review-bot check srr` dé el visto bueno (✅).
Los detalles completos para el tratamiento por parte del rol de LE de los envíos de software estadístico se facilitan en el correspondiente [*Guía de desarrollo de estadísticas* capítulo](https://stats-devguide.ropensci.org/pkgsubmission.html#editor-in-chief).

### El dashboard editorial de rOpenSci {#eic-dashboard}

En [*dashboard Editorial de rOpenSci*](https://dashboard.ropensci.org) se actualiza a diario, principalmente extrayendo información de todos los issues de revisión de software en GitHub, junto con información adicional de Slack y de nuestra base de datos Airtable.
El panel proporciona una visión general actualizada de nuestro equipo editorial, sus responsabilidades recientes y el estado actual de todos los problemas de revisión de software.
La persona con rol de LE (o cualquier editor que esté interesado) puede obtener una visión general del estado del equipo editorial, su disponibilidad y las cargas de trabajo recientes en [página sobre *editores*](https://dashboard.ropensci.org/editors.html).
Debe utilizarse para encontrar y asignar editores para los nuevos temas de revisión de software.
En la [página *Revisión de software*](https://dashboard.ropensci.org/reviews.html) se encuentran las entradas coloreadas por una medida de "urgencia", resumida en la sección [tabla al final de esa página](https://dashboard.ropensci.org/reviews.html#urgency-of-reviews).

Las tareas para las revisiones en las fases de revisión específicas incluyen:

- Revisar los envíos en `0/editorial-team-prep` y `1\/editorial-team-prep`, para comprobar si hay que tomar alguna medida (como sondear al equipo editorial, tomar decisiones, dejar temas en suspenso, hacer ping para actualizaciones, o buscar y asignar editores).

- Revisa los envíos en `2\/seeking-reviewer(s)` para asegurarte de que todo avanza con rapidez.
  Si la búsqueda de revisores se prolonga durante un tiempo inusualmente largo (color rojo), comprueba si el envío está en espera, lee el hilo para reunir contexto y ponte en contacto con la persona encargada en privado para pedirle más información.

- Revisa los envíos en `3\/reviewer(s)-assigned`.
  Si siguen faltando revisiones después de un tiempo inusualmente largo (color rojo), comprueba si el envío está en espera, lee el hilo para recopilar el contexto y ponte en contacto con el editor en privado para pedirle más información.

- Revisa los envíos en 4\/review(s)-in-awaiting-changes`.
  Si algunos siguen sin respuesta de las autoras después de un tiempo inusualmente largo (color rojo), comprueba si el envío está en espera, lee el hilo y ponte en contacto con la persona encargada en privado para pedir más información.

### Invitar a una persona para la tarea de edición  {#guesteditor}



Tras debatirlo con otros miembros del equipo, quien sea LE puede invitar a una persona externa para que se encargue de un envío.
Los editores invitados pueden ser deseables o necesarios  si el volumen de envíos es grande, si todos los miembros del equipo de edición tienen un conflicto de intereses, si se necesitan conocimientos específicos, o como prueba antes de enviar una invitación a formar parte del equipo de edición.

Para decidir a quién invitar para un envío:

- Crea un canal privado de Slack llamado #editor-invitado<nombre-paquete>.
- Invita al equipo editorial es a él.
- Busca en canales slack similares archivados posibles recomendaciones.
...
- Pide a un miembro del personal que añada su nombre a la tabla de editores invitados de Airtable (para que sus nombres aparezcan en este libro y en el README de la revisión del software).
...
Las editoras encargadas son responsables de guiar cada envío asignado a lo largo de todo el proceso, desde la presentación inicial hasta la aceptación final.
A cada persona del equipo editorial no se les debe asignar más de un número por trimestre, o un máximo de cuatro al año.
...
- El envío generará automáticamente una salida de comprobación de paquetes del robot de revisión ropensci.
  Todos los fallos de comprobación (❌) deben rectificarse antes de continuar, aunque pueden justificarse excepciones a discreción del editor encargado de la tramitación.
  Anima a quién envió a activar las comprobaciones llamando a `@ropensci-review-bot check package` para confirmar que todas las comprobaciones son correctas (✅).
- Examina todos los elementos marcados con 👀, y solicita más información o medidas al autor cuando proceda.
...
- Comprueba que la plantilla de incidencias se ha rellenado correctamente. Los descuidos y omisiones más comunes deberían ser detectados y anotados por el robot, pero una comprobación manual siempre ayuda.
  Las editoras pueden editar las plantillas directamente para correcciones menores, o pueden indicar a los autores que rellenen los campos obligatorios de la plantilla que puedan faltar.
...
El informe automatizado también incluirá un enlace a un *visualización interactiva en red de las llamadas entre objetos del paquete*.
Esta visualización puede proporcionar una visión útil de cómo están estructuradas las funciones dentro de un paquete.
...
- Un paquete es extremadamente grande, en cuyo caso puede resultar demasiado pesado para los revisores.
  Por ejemplo, [estas consultas iniciales](https://github.com/ropensci/software-review/issues/725#issuecomment-3326436119) revelaron un paquete con más de 8.500 líneas de código R (correspondientes al 97,6% de todos los paquetes), y 251 funciones R exportadas.

#### Comentarios editoriales iniciales

- Después de contabilizar las comprobaciones automáticas, utiliza [plantilla del editor](#editortemplate) para guiar las comprobaciones iniciales (si no están ya cubiertas por la personal con el rol de LE) y registrar tu respuesta al envío.
  También puedes agilizar las comprobaciones editoriales utilizando la plantilla [`pkgreviewr` paquete creado por Anna Krystalli cuando era parte del equipo editorial](https://docs.ropensci.org/pkgreviewr/articles/editors.html).
  Procura terminar las comprobaciones y empezar a buscar revisores en un plazo de 5 días laborables.
- Comprueba las políticas [de alcance](#aims-and-scope) y [solapamiento](#overlap). Inicia un debate a través del canal Slack #software-review si es necesario en casos especiales que no hayan sido detectados por comprobaciones previas. Si se rechaza el paquete, ver [esta sección](#outofscoperesponse) sobre cómo responder.
  Inicia el debate a través del canal Slack #software-review si es necesario para casos extremos que no hayan sido detectados por comprobaciones previas de LE.
...
- Siempre que sea posible, cuando pidas cambios, sugiere herramientas automáticas como [usethis](https://usethis.r-lib.org/), [Air](https://posit-dev.github.io/air/), [Jarl](https://jarl.etiennebacher.com/), [flirtea](https://flir.etiennebacher.com/), y a recursos en línea (secciones de esta guía, secciones de la [Libro de paquetes R](https://r-pkgs.org/)) para facilitar tu devolución. [Ejemplo de comprobaciones de una editora](https://github.com/ropensci/software-review/issues/207#issuecomment-379909739).
...
Estos son los criterios que debes tener en cuenta a la hora de elegir a un revisor. Puede que tengas que reunir esta información buscando en [r-universo](https://r-universe.dev) y la página GitHub del posible revisor, así como su presencia general en Internet (sitio web personal, redes sociales).
...
- Asigna a cada revisor `@ropensci-review-bot assign @username as reviewer`.
  Debe emitirse una orden para cada revisora.
  Si es necesario más adelante, elimina a revisores con `@ropensci-review-bot remove @username from reviewers`.
...
- Una vez enviada cada revisión,
  ...
  - Escribe un comentario de agradecimiento al revisor con tus propias palabras.
...
Tanto autores como revisores pueden pedir opiniones sobre aspectos del proceso de revisión, como por ejemplo el tono o el contenido de las revisiones o las respuestas de los autores.
...
- **Si quién envió el paquete deja de responder** remítete a [las políticas](#policies) y/o ponte en contacto con los demás editores en el canal Slack para debatirlas.
  Es importante que, si se asignó una revisora a un tema cerrado, te pongas en contacto con esta esta persona al cerrar el tema para explicarle la decisión y agradecerle de nuevo su trabajo.
  Informa al equipo editorial en el canal de Slack para que los tengan en cuenta como revisores para un paquete en el futuro con altas posibilidades de revisión de software sin problemas.
- **Si alguién se retrasa en la revisión o deja de responder** envía un recordatorio al cabo de 1 semana, y de nuevo al cabo de 2 semanas.
  ...
  Si al cabo de 3 semanas sigue sin haber respuesta, determina cuál es la mejor manera de seguir adelante sin ellos:
  ...
  - Si la persona ausente no ha enviado su revisión, el editor debe intentar encontrar un nuevo revisor, y proceder con el proceso de revisión una vez que haya dos revisiones.
    En este punto, el editor debe dar prioridad a la búsqueda de revisores experimentados que puedan comprometerse a una entrega rápida.
    Asegúrate de hacer ping a otros editores en Slack.
    - A su discreción, el editor *puede* optar por actuar él mismo como segundo revisor, pero sólo debe hacerlo tras múltiples intentos fallidos de encontrar un nuevo revisor, y si el editor tiene experiencia suficiente para hacerlo.
      Desaconsejamos a los editores que hagan esto con cierta frecuencia, ya que aumenta la carga de trabajo y reduce la diversidad de puntos de vista aportados a la comunidad por los revisores.
    - Haz un comentario dando las gracias al revisor original en cualquier caso, y elimínalo con `@ropensci-review-bot remove @username from reviewers`.

### Después de la revisión {#after-review}

- Aprobar el paquete con `@ropensci-review-bot approve <package-name>`
- Consulta la [sección siguiente]{#non-transferred-packages} para saber cómo permitir a autores mantener el paquete en su propia organización de GitHub si prefieren no transferirlo a rOpenSci.
- Nombra un paquete para que aparezca en una entrada del blog de rOpenSci o en una nota técnica si crees que puede ser de gran interés.
  Por favor, anota en el tema de la revisión del software una o dos cosas que el autor podría destacar, y etiqueta `@ropensci/blog-editors` para su seguimiento.
- Si quién envió el paquete mantiene un libro en línea que trata, al menos en parte, de su paquete, ponte en contacto con [alguién del personal de rOpenSci](https://ropensci.org/about/#team) para que se ponga en contacto con los autores sobre la transferencia a [la p `ropensci-books` organización de GitHub](https://github.com/orgs/ropensci-books).

#### Paquetes que permanecen en las organizaciones originales de GitHub {#non-transferred-packages}

Para los autores de paquetes que deseen mantener sus repositorios en sus organizaciones originales de GitHub, en lugar de transferirlos a github.com/ropensci, los editores deben:

- Pedir a los autores de los paquetes que hagan un pull request a la organización [archivo JSON](https://github.com/ropensci/roregistry/blob/gh-pages/info/not_transferred.json) que enumera todos los repositorios que no han sido transferidos. [Ejemplo de transferencia](https://github.com/ropensci/roregistry/commit/9568a77c2081f89d58406efcc9089ef83d09fe59).

- Pedir al paquete que sustituya el contenido del código de conducta actual del repositorio por el contenido del [código de conducta por defecto de la organización rOpenSci GitHub](https://github.com/ropensci/.github/blob/main/CODE_OF_CONDUCT.md).

#### Promoción de paquetes {#package-promotion}

- Dirige a quién envió el paquete a los capítulos de la guía sobre [liberación de paquetes](#releases), [marketing](#marketing) y [preparación para GitHub](#grooming).
