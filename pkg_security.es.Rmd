...
Esta sección contiene orientaciones para cuando desarrolles un paquete que interactúe con un recurso web que requiera credenciales (claves API, tokens, etc.). Consulta también [el sitio web `httr` sobre cómo compartir secretos](https://httr.r-lib.org/articles/secrets.html).

### Secretos en paquetes y protección de usuarios

Supongamos que tu paquete necesita una clave API para hacer peticiones en nombre de los usuarios de tu paquete.

- En la documentación de tu paquete, orienta al usuario para que la clave de la API no acabe en el .Rhistory/script de los usuarios de tu paquete.
  
  - Fomenta el uso de variables de entorno para almacenar la clave API (¿o incluso eliminar la posibilidad de pasarla como argumento a las funciones?) Podrías enlazar [a esta introducción a los archivos de inicio](https://rstats.wtf/r-startup.html) y [`usarethis::edit_r_environ()`](https://usethis.r-lib.org/reference/edit.html).
  
  - O tu paquete podría depender de, o fomentar el uso de [`llavero` para ayudar al usuario a almacenar las variables](https://github.com/r-lib/keyring#readme) en los almacenes de credenciales del SO específico (más seguro que .Renviron): es decir, crearías una función para establecer la clave, y tendrías otra para recuperar la clave; o el usuario escribiría `Sys.setenv(SUPERSECRETKEY = keyring::key_get("myservice"))` al principio de su script.
  
  - No imprimas la clave de la API ni siquiera en modo verbose en ningún mensaje, advertencia o error.

- En la plantilla de emisión de GitHub, debe indicarse que no se compartan las credenciales. Si un usuario de tu paquete comparte accidentalmente las credenciales en una incidencia, asegúrate de que es consciente de ello para que pueda revocar la clave (es decir, pregúntale explícitamente en una respuesta si se ha dado cuenta de que ha compartido su clave).
...
Si utilizas [`vcr`](https://docs.ropensci.org/vcr/) o [`httptest`](https://enpiar.com/r/httptest/) en las pruebas para almacenar en caché las respuestas de la API, tienes que asegurarte de que las peticiones grabadas / accesorios no contienen secretos. Consulta [`vcr` guía de seguridad](https://books.ropensci.org/http-testing/security-chapter.html) y [`httptest` orientación "Redactar y modificar las solicitudes grabadas"](https://enpiar.com/r/httptest/articles/redacting.html)e inspecciona tus peticiones grabadas / accesorios antes de comprometerlos la primera vez para asegurarte de que tienes la configuración correcta.

`vcr` al ser un paquete de rOpenSci, puedes publicar cualquier pregunta que tengas en [foro de rOpenSci](https://discuss.ropensci.org/).

#### Compartir secretos con los servicios de IC

Ahora, es posible que necesites compartir secretos con [servicios de integración continua](#ci).

Podrías almacenar las claves de la API como variables de entorno / secretos, consultando la documentación del servicio de IC.
...
[Precompleta las viñetas](https://ropensci.org/technotes/2019/12/08/precompute-vignettes/) que requieren credenciales.
...
- [el `notario` paquete](https://ropensci.org/blog/2017/07/25/notary/);

- [`gárgola` artículo "Gestionar los tokens de forma segura"](gargle.r-lib.org/articles/articles/managing-tokens-securely.html)
